...
def plotfig(targets, predictions, sw_scores, pre_title, save_dir):
    """
    ...
    plt.legend()
    plt.savefig(save_dir + '/' + f'{pre_title} - Predicted vs Actual Values.png')

    # 绘制 Residuals 分布图
    plt.figure(figsize=(12, 6))
    plt.hist(residuals, bins=50, color='skyblue', edgecolor='black')
    plt.title(f'{pre_title} - Relative Error Distribution')
    plt.xlim(-0.5,0.5)
    plt.ylim(0,12000)
    plt.xlabel('(Actual - Predicted)/|Actual|')
    plt.ylabel('Frequency')
    plt.savefig(save_dir + '/' + f'{pre_title} - Relative Error Distribution.png')
    ...
    plt.legend()
    plt.colorbar(scatter, label='SW_score')
    plt.savefig(save_dir + '/' + f'{pre_title} - Error vs Actual Values.png')
