...
class SignalDataset(Dataset):
    ...
    @staticmethod
    @torch.jit.script
    def default_phase_correction(iq_signal: Tuple[torch.Tensor, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        """默认IQ相位校正：利用信号均值进行相位旋转"""
        i_data, q_data = iq_signal
        complex_signal = torch.complex(i_data, q_data)

        if torch.abs(complex_signal).max() < 1e-6:
            return iq_signal
        avg_phase = torch.angle(torch.mean(complex_signal))
        cos_phase = torch.cos(avg_phase)
        sin_phase = torch.sin(avg_phase)
        
        corrected_i = i_data * cos_phase + q_data * sin_phase
        corrected_q = -i_data * sin_phase + q_data * cos_phase
